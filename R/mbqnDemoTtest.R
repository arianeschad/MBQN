...
#' of different normalizations to a simulated, differentially expressed and distorted RI feature.
#' @param show.fig Logical that indicates whether results are plot to pdf.
#' @details Apply a two-sided t-test to an RI feature before and after normalization with MBQN.
...
mbqnDemoTtest <- function(show.fig = FALSE){
  ...
  if(show.fig){
    ...
    par(mfrow = c(1,1))
    mbqnBoxplot(mtx,ylim = c(25,36), vals = data.frame(RI = mtx[1,],NRI = mtx[as.numeric(names(res$nri)[2]),]), y.intersp= 1.5)
    dev.copy2pdf(file=file.path(getwd(),"fig_undistorted_boxplot_simu.pdf"),width=8,height=4,paper="a4r",out.type = "pdf")

    #dev.off()
    mbqnBoxplot(mtx.mod,ylim = c(25,39), vals = data.frame(RI = mtx.mod[1,],NRI = mtx.mod[as.numeric(names(res$nri)[2]),]), y.intersp= 1.5)
    dev.copy2pdf(file=file.path(getwd(),"fig_distortion_boxplot_simu.pdf"),width=8,height=4,paper="a4r",out.type = "pdf")

    #dev.off()
    mbqnBoxplot(qn.mtx,ylim = c(25,36), vals = data.frame(RI = qn.mtx[1,],NRI = qn.mtx[as.numeric(names(res$nri)[2]),]), y.intersp= 1.5)
    dev.copy2pdf(file=file.path(getwd(),"fig_normalized_boxplot_simu.pdf"),width=8,height=4,paper="a4r",out.type = "pdf")

    #dev.off()
    mbqnBoxplot(mbqn.mtx,ylim = c(25,36), vals = data.frame(RI = mbqn.mtx[1,],NRI = mbqn.mtx[as.numeric(names(res$nri)[2]),]), y.intersp= 1.5)
  ...
  if(ttest.res$p.value<0.05)
   ...
   return(list(bla$x.mod, bla$mx.offset, bla$mx.scale,ttest.undistorted = ttest.res0, ttest.distorted = ttest.res1, ttest.mbqndistorted = ttest.res))
