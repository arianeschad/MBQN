#' Simulate an omics-like intensity matrix with mean and scale distorted samples
#'
#' @description Illustrate the influence of normalization on expression analysis.
#' Distort columnwise mean and scale of a simulated, omics-like matrix with a single differentially
#' expressed NRI feature. Apply a two-sided t-test before and after application of different normalizations.
...
#' @param flag.fig Logical that indicates whether results are plot to pdf, default FALSE
#' @details Shift and scale the sample mean and standard deviation of a simulated matrix of structured, random
...
#' \eqn{\sigma_scale}=\code{max_scale}, respectively. A single, rank invariant (RI) feature with differential expression is stacked to the matrix before mean and scale distortion. A two-sided t-test
#' is applied to the RI feature before and after normalization with QN and MBQN.
#' @return \code{list} of distorted matrix of size nrow x ncol, arrays of mean and scale distortions, and figures in pdfs
...
#' @examples
#'\dontrun{
#' MBQN:::mbqnSimuDistortion(ncol = 18)
#' }
#' @author A. Schad, \email{ariane.schad@zbsa.de}
# #' @export mbqnSimuDistortion
mbqnSimuDistortion <- function(ncol = 18,max_mean = 0.05, max_scale = 0.01, seed = 1234, flag.fig = FALSE){
  ...
  return(distort = list(mtx = mtx, mx.offset = mx.offset, mx.scale = mx.scale))
