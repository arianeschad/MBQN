#' Test if normalization of data benefits from mean-balancing
#'
#' Compute a quantile normalization without and with mean-balancing and check standard deviation of normalized data entries located in the tails
...
mbqn.check_saturation <- function(dat, FUN = mean_fun, qlow, qup){
  ...
  qn.dat <- normalize.quantiles(dat)
  ...
  mbqn.dat <- normalize.quantiles(dat-mdat) + mdat
