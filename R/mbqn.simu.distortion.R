# #' Simulate distortion of mean and scale to a data matrix
# #'
# #' @description Simulate an omics-like, columnwise distortion in mean and scale of a data matrix
...
# #' @examples
# #' mbqn.simu.distortion(ncol = 9)
# #' @author A. Schad, \email{ariane.schad@zbsa.de}
# #' @export mbqn.simu.data
mbqn.simu.distortion <- function(ncol = NULL,max_mean = 0.05, max_scale = 0.01, seed = 1234){
  ...
  res <- mbqn.check_saturation(mtx.mod,FUN= "mean", low_thr = 0.5, verbose = F, show_nri_only = T, y.intersep = 0.8, save_fig = T)
  ...
  dev.off()
  mbqn.boxplot(mtx,ylim = c(22,36), vals = data.frame(RI = mtx[1,],NRI = mtx[as.numeric(names(res$nri)[2]),]), y.intersp= 0.8)
  dev.copy2pdf(file=file.path(getwd(),"fig_undistorted_boxplot_simu.pdf"),width=8,height=4,paper="a4r",out.type = "pdf")

  dev.off()
  mbqn.boxplot(mtx.mod,ylim = c(21,39), vals = data.frame(RI = mtx.mod[1,],NRI = mtx.mod[as.numeric(names(res$nri)[2]),]), y.intersp= 0.8)
  dev.copy2pdf(file=file.path(getwd(),"fig_distortion_boxplot_simu.pdf"),width=8,height=4,paper="a4r",out.type = "pdf")

  dev.off()
  mbqn.boxplot(qn.mtx,ylim = c(22,36), vals = data.frame(RI = qn.mtx[1,],NRI = qn.mtx[as.numeric(names(res$nri)[2]),]), y.intersp= 0.8)
  dev.copy2pdf(file=file.path(getwd(),"fig_normalized_boxplot_simu.pdf"),width=8,height=4,paper="a4r",out.type = "pdf")

  dev.off()
  mbqn.boxplot(mbqn.mtx,ylim = c(22,36), vals = data.frame(RI = mbqn.mtx[1,],NRI = mbqn.mtx[as.numeric(names(res$nri)[2]),]), y.intersp= 0.8)
