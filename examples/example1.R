...
# See "SafeQuant"
# by Erik Ahrne (2016). SafeQuant: A Toolbox for the Analysis of Proteomics Data. R package version 2.3.1.
# https://CRAN.R-project.org/package=SafeQuant


# Usage:
# > source("examples/example1.R")
# > example1(which.example)

# Arguments:
# which.example - select an example between 1-4
...
example1 <- function(which.example = 3){
  ...
  file <- file.path("examples",ids[which.example],"proteinGroups.txt")
  if(!file.exists(file)){
    ...
    ifelse(!dir.exists(file.path("examples",ids[which.example])), dir.create(file.path("examples",ids[which.example])),'')

    if(!file.exists(f[ind])){
      fnm <- rpx::pxget(px, f[ind])
    }
    if(file.exists(f[ind])){ # unzip file
      unzip(fnm,files = "proteinGroups.txt", exdir=file.path("examples",ids[which.example]))
  ...
  ###################################################################################
  # Select columns with LFQ intensities
  ...
  # select columns
  if(pxdid == "PXD005861") {ix <- c(1:9, 19:27)}
